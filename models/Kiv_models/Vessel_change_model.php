...
class Vessel_change_model extends CI_Model
{
...
function get_vessel_details_viewpage($vessel_id)
{
    ...
    $this->db->join('tb_vessel_main f','f.vesselmain_vessel_id=a.vessel_sl');
...
/*function get_vessel_created_user($vessel_sl)       
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_vessel_details a');
    $this->db->join('user_master b','a.vessel_created_user_id=b.user_sl');
...
function get_process_flow($processflow_sl)       
{
    ...
    $this->db->select('a.user_id as curruser');
...
{
    $id=array(1,5,11,12,13,22,23,24,26);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    $this->db->join('tbl_kiv_vessel_details b','a.vessel_id=b.vessel_sl');
    $this->db->join('tbl_kiv_user_vessel d','a.vessel_id=d.vessel_id');
...
{
    $id=array(1,5,7,14,15,18,26,38,39,41);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    $this->db->join('tbl_kiv_vessel_details b','a.vessel_id=b.vessel_sl');
...
function get_vessel_details_owner($id)
{
    ...
    $this->db->join('tbl_kiv_user_vessel d','a.vessel_sl=d.vessel_id');
    /*$this->db->join('tbl_kiv_vessel_timeline b','a.vessel_sl=b.vessel_id');
    $this->db->where('b.registration_number!=','');*/
    $this->db->where('d.user_id',$id);
...
function get_payment_details($pay_sl,$id)
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_payment_details');
    $this->db->where('payment_sl',$pay_sl);
    $this->db->where('vessel_id',$id);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_surveyor_details()
{
    $this->db->select('*');
    $this->db->from('user_master');
    $this->db->where_in('user_master_id_user_type',5);
    //$this->db->where('delete_status',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_usertype_master($user_type_id)
{
    ...
    $this->db->where('user_type_sl',$user_type_id);
     $this->db->where('delete_status',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_vessel_owner_name($sess_usr_id)
{
    ...
    $this->db->select('b.user_master_fullname');
    $this->db->from('tbl_kiv_user a');
    $this->db->join('user_master b','a.user_sl=b.customer_id');
    $this->db->where('b.user_master_id',$sess_usr_id);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result; 
}
...
function get_state($state_id)
{
    ...
    $this->db->where('state_sl',$state_id);
...
function get_registration_intimation($vessel_id,$intimation_type_id)
{
    ...
    $this->db->join('tbl_kiv_vessel_details b','a.vessel_id=b.vessel_sl');
    $this->db->join('tb_vessel_main f','f.vesselmain_vessel_id=b.vessel_sl');
...
function get_survey_done($process_id,$initial_survey_id)
{
    $this->db->select('*');
...
///////////////////////Deepthi(start)//////////////////////////////////////////////
/////////////////vesselnamechange----start-------//////////////


function get_vesselchange_List($user_id)       
{
    $this->db->select('*');   
   // $this->db->select('tbl_kiv_vessel_details.*');
    $this->db->select('tb_vessel_main.*');
    $this->db->from('tb_vessel_main');
    $this->db->join('tbl_kiv_status_details','tb_vessel_main.vesselmain_vessel_id=tbl_kiv_status_details.vessel_id'); 
    //$this->db->join('tbl_kiv_vessel_details','tbl_kiv_vessel_details.vessel_sl=tbl_kiv_processflow.vessel_id');    
    //$this->db->join('kiv_vesseltype_master','kiv_vesseltype_master.vesseltype_sl=tbl_kiv_vessel_details.vessel_type_id','left');
    //$this->db->join('tbl_kiv_processname','tbl_kiv_processname.processname_sl=tbl_kiv_processflow.process_id');
    /*$this->db->join('tbl_kiv_vessel_details','tbl_kiv_vessel_details.vessel_sl=tb_vessel_main.vesselmain_vessel_id');
     $this->db->join('tbl_kiv_processflow','tbl_kiv_vessel_details.vessel_sl=tbl_kiv_processflow.vessel_id'); 
    $this->db->join('tbl_kiv_processname','tbl_kiv_processname.processname_sl=tbl_kiv_processflow.process_id');*/
    $this->db->where('tb_vessel_main.vesselmain_reg_number<>','0');
    $this->db->where('tb_vessel_main.vesselmain_owner_id',$user_id);
    //$this->db->where('tbl_kiv_processflow.process_id',38);
    //$this->db->where('tbl_kiv_processflow.status',1);
    /*$this->db->select('*');   
    $this->db->from('tb_vessel_main');
    $this->db->where('vesselmain_reg_number<>','0');
    $this->db->where('vesselmain_owner_id',$user_id);*/
...
function get_vesselDet($vessel_id)
{
    ...
    $this->db->select('vessel_total_tonnage');
	$this->db->select('vessel_registration_number');
...
function get_form11_frwd_process_flow($user_id,$vessel_id)       
{
    //$id=array(38);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    ...
    $this->db->where('a.vessel_id',$vessel_id);
    //$this->db->where_in('a.process_id',$id);
    //$this->db->where('a.current_status_id',5);
    $this->db->where('a.status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_tariff_tonnagetype($activity_id,$form_id,$vessel_type_id,$vessel_subtype_id)
{
    $this->db->select('*');
    $this->db->from('kiv_tariff_master');
    $this->db->where('tariff_activity_id',$activity_id);
...
function get_tariff_form11($survey_id,$form_id,$vessel_type_id,$vessel_subtype_id)
{
    $this->db->select('*');
    $this->db->from('kiv_tariff_master');
    $this->db->where('tariff_activity_id',$survey_id);
    ...
    $this->db->where('tariff_vessel_subtype_id',$vessel_subtype_id);
    $this->db->where('delete_status',0);
    /*if($tariff_tonnagetype_id==2){
        $this->db->where("$vessel_total_tonnage between tariff_from_ton and tariff_to_ton", NULL, FALSE );
    } else if($tariff_tonnagetype_id==3){
        $this->db->where("$vessel_total_tonnage between tariff_from_ton and tariff_to_ton", NULL, FALSE );
        $this->db->where("$numberofdays between tariff_from_day and tariff_to_day", NULL, FALSE );
    }*/
...
function get_vesselnamechange_details($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_namechange.change_vessel_id');
    $this->db->join('tbl_kiv_vessel_details','tb_vessel_main.vesselmain_vessel_id=tbl_kiv_vessel_details.vessel_sl');
    $this->db->join('user_master','user_master.user_master_id=tb_vessel_main.vesselmain_owner_id');
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    //$this->db->where('tbl_namechange.change_status',1);
    $this->db->where('tbl_kiv_vessel_details.vessel_created_user_id',$usr);
    $where= "tbl_namechange.change_status='1' OR tbl_namechange.change_status='2'";
    $this->db->where($where);
    
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
    $this->db->select('e.user_type_name');
    $this->db->from('tbl_kiv_vessel_details a');
    $this->db->join('tb_vessel_main f','f.vesselmain_vessel_id=a.vessel_sl');
    $this->db->join('tbl_kiv_status_details b','a.vessel_sl=b.vessel_id');
    $this->db->join('user_master d','d.user_sl=b.receiving_user_id');
    //$this->db->join('tbl_kiv_user c','c.user_sl=d.customer_id');
    $this->db->join('user_type e','e.user_type_sl=d.user_type_id');
    $this->db->where('b.process_id',38);
    $this->db->where('b.survey_id',0);
    $this->db->order_by('a.vessel_sl','ASC');*/
  //$this->db->limit(3,0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
...
function get_duplicatecert_details($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_duplicate_certificate.duplicate_cert_vessel_id');
    $this->db->join('tbl_kiv_vessel_details','tb_vessel_main.vesselmain_vessel_id=tbl_kiv_vessel_details.vessel_sl');
    $this->db->join('user_master','user_master.user_master_id=tb_vessel_main.vesselmain_owner_id');
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    //$this->db->where('tbl_namechange.change_status',1);
    $this->db->where('tbl_kiv_vessel_details.vessel_created_user_id',$usr);
    $where= "tbl_duplicate_certificate.duplicate_cert_status='1' OR tbl_duplicate_certificate.duplicate_cert_status='2'";
    $this->db->where($where);
...
function get_vesselnamechange_details_ra_vw($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_namechange.change_vessel_id');
    ...
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_namechange.change_status',1);
    $this->db->where('tbl_kiv_status_details.receiving_user_id',$usr);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
...
function get_duplicatecert_details_ra_vw($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_duplicate_certificate.duplicate_cert_vessel_id');
    ...
    $this->db->join('user_master','user_master.user_master_id=tb_vessel_main.vesselmain_owner_id');
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_duplicate_certificate.duplicate_cert_status',1);
    $this->db->where('tbl_kiv_status_details.receiving_user_id',$usr);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
...
function get_vesselnamechange_details_vw($id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_namechange.change_vessel_id');
    ...
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_namechange.change_status',1);
    $this->db->where('tbl_namechange.change_vessel_id',$id);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
...
function get_vesselnamechangestatus_details($vessel_id)
{
...
function get_vesselnamechange_details_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_namechange.change_vessel_id');
...
function get_vesselnamechange_payment_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_payment_details');
    $this->db->where('vessel_id',$id);
    $this->db->where('form_number',11);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function update_namechange($table,$data,$id)
{
    $where      =   "change_vessel_id  = $id AND change_status=1"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
} 
function get_owner($id)
{
    ...
    $this->db->select('a.user_name');
    $this->db->select('a.user_address');
    $this->db->from('tbl_kiv_user a');
    $this->db->join('user_master b','a.user_sl=b.customer_id');
    $this->db->where('b.user_master_id',$id);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_namechange_intimation_det($vessel_id,$survey_id,$process_id,$status)       
{
    //$id=array(38);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    $this->db->join('tbl_kiv_vessel_details b','a.vessel_id=b.vessel_sl');
    $this->db->join('a5_tbl_registration_intimation c','c.vessel_id=b.vessel_sl');
    $this->db->where('a.process_id',$process_id);
    $this->db->where('a.vessel_id',$vessel_id);
    $this->db->where('a.survey_id',$survey_id);
    $this->db->where('a.status',1);
    $this->db->where('a.current_status_id',7);
    $this->db->where('c.registration_intimation_type_id',2);
    $this->db->where('c.registration_inspection_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function check_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange_log');
    $this->db->where('vessel_id',$id);
    $this->db->where('status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
...
function check_vessel_nmchg($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->where('change_vessel_id',$id);
    $this->db->where('change_vessel_main_id',$main_id);
    $this->db->where('change_status',1);
    $this->db->where('change_approve_id<>',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function name_change_det($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->where('change_vessel_id',$id);
    $this->db->where('change_vessel_main_id',$main_id);
    $this->db->where('change_status',1);
    $this->db->where('change_verify_id',0);
    $this->db->where('change_approve_id',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function name_change_details($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->where('change_vessel_id',$id);
...
function check_vessel_nmchg_pay($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange');
    $this->db->where('change_vessel_id',$id);
...
function check_vessel_ownchg_pay($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$id);
    $this->db->where('transfer_vessel_main_id',$main_id);
    $this->db->where('transfer_changetype ',2);
    $this->db->where('transfer_status',1);
    $this->db->where('transfer_approve_id',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function check_vessel_dupcert_pay($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->where('duplicate_cert_vessel_id',$id);
    $this->db->where('duplicate_cert_vessel_main_id',$main_id);
    $this->db->where('duplicate_cert_status',1);
    $this->db->where('duplicate_cert_decisionmake_id',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function check_vessel_transfer_pay($id,$main_id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$id);
    $this->db->where('transfer_vessel_main_id',$main_id);
    $this->db->where('transfer_changetype ',1);
    $this->db->where('transfer_status',1);
    $this->db->where('transfer_approve_id',0);
    $query  =   $this->db->get();
    $result =   $query->result_array();
...
function update_namelog_status($table,$data_log,$vessel_id)
{
    $where      =   "vessel_id  = $vessel_id"; 
    $updquery   =   $this->db->update_string($table, $data_log, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_vesselmain($table,$data,$vesselmain_sl)
{
    $where      =   "vesselmain_sl  = $vesselmain_sl"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_vessel_namechg($table,$data_namechg,$vessel_id,$vesselmain_sl)
{
    $where      =   "change_vessel_id  = $vessel_id AND change_vessel_main_id = $vesselmain_sl AND change_status=1"; 
    $updquery   =   $this->db->update_string($table, $data_namechg, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_vessel_ownchg($table,$data_ownchg,$vessel_id,$vesselmain_sl)
{
    $where      =   "transfer_vessel_id  = $vessel_id AND transfer_vessel_main_id = $vesselmain_sl AND transfer_status=1"; 
    $updquery   =   $this->db->update_string($table, $data_ownchg, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_vessel_duplcert($table,$data_dupli,$vessel_id,$vesselmain_sl)
{
    $where      =   "duplicate_cert_vessel_id  = $vessel_id AND duplicate_cert_vessel_main_id = $vesselmain_sl AND duplicate_cert_status=1"; 
    $updquery   =   $this->db->update_string($table, $data_dupli, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
...
function update_tbl_user_vessel($table,$data,$id)
{
    $where      =   "vessel_id  = $id"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
...
function update_tbl_vessel_main($table,$data,$vessel_id,$vesselmain_sl)
{
    $where      =   "vesselmain_vessel_id  = $vessel_id AND vesselmain_sl = $vesselmain_sl"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function getnamechange_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_namechange_log');
    $this->db->where('vessel_id',$id);
...
function getownerchange_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange_log');
    $this->db->where('vessel_id',$id);
    $this->db->where('change_type',2);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function gettransfer_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange_log');
    $this->db->where('vessel_id',$id);
...
function getduplicatecert($id)
{
    $this->db->select('tbl_duplicate_certificate.*');
    $this->db->select('tb_vessel_main.*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_duplicate_certificate.duplicate_cert_vessel_id');
    $this->db->where('tbl_duplicate_certificate.duplicate_cert_vessel_id',$id);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
////////////////vesselchange-----end-------///////////////
/////////////////vesselownershipchange----start-------//////////////


function get_vesselownerchange_List($user_id)       
{
     $this->db->select('*');   
   // $this->db->select('tbl_kiv_vessel_details.*');
    $this->db->select('tb_vessel_main.*');
    ...
    $this->db->join('kiv_vesseltype_master','kiv_vesseltype_master.vesseltype_sl=tbl_kiv_vessel_details.vessel_type_id','left');
    $this->db->join('tbl_kiv_processname','tbl_kiv_processname.processname_sl=tbl_kiv_processflow.process_id');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_kiv_vessel_details.vessel_sl');
    $this->db->where('tb_vessel_main.vesselmain_reg_number<>','0');
    $this->db->where('tbl_kiv_processflow.user_id',$user_id);
    $this->db->where('tbl_kiv_processflow.status',1);
...
function get_owner_check($mob,$mail)       
{
    ...
    $this->db->from('user_master a');
    $this->db->join('tbl_kiv_user b','a.customer_id=b.user_sl');
    $this->db->where('a.user_master_id_user_type',2);
...
function get_buyer_details($vessel_id)       
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
    $this->db->where('transfer_vessel_id',$vessel_id);
    $this->db->where('transfer_status',1);
    $this->db->where('transfer_changetype',2);
    $query  =   $this->db->get();
    $result =   $query->result_array();
...
function get_dupcert_details($vessel_id)       
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->where('duplicate_cert_vessel_id',$vessel_id);
    //$this->db->where('duplicate_cert_type',$ctype);
    $this->db->where('duplicate_cert_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_buyer_details_pc($vessel_id)       
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$vessel_id);
...
function get_transfervessel_details($vessel_id)       
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
...
function get_transfervessel_details_pc($vessel_id)       
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$vessel_id);
...
function update_ownershipchange($table,$data,$id)
{
    $where      =   "transfer_vessel_id  = $id AND transfer_status=1"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_duplicatecert($table,$data,$id)
{
    $where      =   "duplicate_cert_vessel_id  = $id AND duplicate_cert_status=1"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function get_vesselownerchange_details($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
    ...
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_kiv_vessel_details.vessel_created_user_id',$usr);
    $this->db->where('tbl_transfer_ownershipchange.transfer_changetype',2);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
...
function get_transfervesselstatus_details($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
    ...
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_kiv_vessel_details.vessel_created_user_id',$usr);
    $this->db->where('tbl_transfer_ownershipchange.transfer_changetype',1);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
...
function get_transfervesseloutside_details($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
    ...
    $this->db->where('tbl_kiv_vessel_details.vessel_created_user_id',$usr);
    $this->db->where('tbl_transfer_ownershipchange.transfer_changetype',1);
    $this->db->where('tb_vessel_main.processing_status',2);
    $this->db->where('tbl_transfer_ownershipchange.transfer_based_changetype',0);
    //$this->db->where('tbl_kiv_status_details.process_id',38);
    /*$this->db->select('a.*');
    $this->db->select('b.current_status_id ');
    $this->db->select('b.receiving_user_id');
...
function get_vesselownerchange_details_ra_vw($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
...
function get_vesseldupcert_details_ra_vw($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_duplicate_certificate.duplicate_cert_vessel_id');
...
function get_transfervessel_details_ra_vw($usr)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
...
function get_ownerchange_intimation_det($vessel_id,$survey_id,$process_id,$status)       
{
    //$id=array(38);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    ...
    $this->db->where('a.status',1);
    $this->db->where('a.current_status_id',7);
    $this->db->where('c.registration_intimation_type_id',3);
    $this->db->where('c.registration_inspection_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_dupcert_intimation_det($vessel_id,$survey_id,$process_id,$status)       
{
    //$id=array(38);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    ...
    $this->db->where('a.status',1);
    $this->db->where('a.current_status_id',7);
    $this->db->where('c.registration_intimation_type_id',5);
    $this->db->where('c.registration_inspection_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_transfervessel_intimation_det($vessel_id,$survey_id,$process_id,$status)       
{
    //$id=array(38);
    $this->db->select('*');
    $this->db->from('tbl_kiv_processflow a');
    ...
    $this->db->where('a.status',1);
    $this->db->where('a.current_status_id',7);
    $this->db->where('c.registration_intimation_type_id',4);
    $this->db->where('c.registration_inspection_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_vesselownerchange_details_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
    $this->db->join('tbl_kiv_vessel_details','tb_vessel_main.vesselmain_vessel_id=tbl_kiv_vessel_details.vessel_sl');
    $this->db->join('user_master','user_master.user_master_id=tbl_kiv_vessel_details.vessel_created_user_id');
    $this->db->join('tbl_kiv_user','user_master.customer_id=tbl_kiv_user.user_sl');
    $this->db->join('user_type','user_type.user_type_id=user_master.user_master_id_user_type');
    $this->db->where('tbl_transfer_ownershipchange.transfer_vessel_id',$id);
...
function get_vesselduplicert_details_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_duplicate_certificate.duplicate_cert_vessel_id');
...
function get_transfervessel_details_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->join('tb_vessel_main','tb_vessel_main.vesselmain_vessel_id=tbl_transfer_ownershipchange.transfer_vessel_id');
...
function get_vesselownerchange_payment_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_payment_details');
    $this->db->where('vessel_id',$id);
    $this->db->where('form_number',18);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_transfervessel_payment_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_payment_details');
    $this->db->where('vessel_id',$id);
    $this->db->where('form_number',19);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_dupcert_payment_ra($id)
{
    $this->db->select('*');
    $this->db->from('tbl_kiv_payment_details');
    $this->db->where('vessel_id',$id);
    $this->db->where('form_number',20);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function checkowner_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange_log');
    $this->db->where('vessel_id',$id);
...
function checktransfer_vessel($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange_log');
    $this->db->where('vessel_id',$id);
...
function update_ownerchg_status($table,$data_own,$vessel_id)
{
    $where      =   "transfer_vessel_id  = $vessel_id AND transfer_changetype=2"; 
    $updquery   =   $this->db->update_string($table, $data_own, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_transfervessel_status($table,$data_own,$vessel_id)
{
    $where      =   "transfer_vessel_id  = $vessel_id AND transfer_changetype=1"; 
    $updquery   =   $this->db->update_string($table, $data_own, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function update_dupcert_status($table,$data_dup,$vessel_id)
{
    $where      =   "duplicate_cert_vessel_id  = $vessel_id"; 
    $updquery   =   $this->db->update_string($table, $data_dup, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function get_profile_status($id,$stat)
{
    $this->db->select('transfer_changepending_status');
    $this->db->select('transfer_based_changetype');
    $this->db->select('transfer_buyer_mobile');
    $this->db->select('transfer_buyer_email_id');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$id);
    $this->db->where('transfer_changetype',$stat);
    $this->db->where('transfer_status',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function update_namechg($table,$data,$vessel_id)
{
    $where      =   "change_vessel_id  = $vessel_id AND payment_status=0 AND change_status=1"; 
    $updquery   =   $this->db->update_string($table, $data, $where);
    $result     =   $this->db->query($updquery);
    return $result; 
}
function get_processflow_details($vessel_sl,$process_id)       
{
    $this->db->select('processflow_sl');
    $this->db->from('tbl_kiv_processflow');
    $this->db->where('vessel_id',$vessel_sl);
    $this->db->where('process_id ',$process_id);
    $this->db->where('survey_id ',0);
    $this->db->where('status ',1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_processflow_details_before_flow($vessel_sl)       
{
    $this->db->select('processflow_sl');
    $this->db->from('tbl_kiv_processflow');
    $this->db->where('vessel_id',$vessel_sl);
...
function get_all_states()
{
    ...
    $this->db->from('kiv_state_master');
    $this->db->where('state_status',1);
    $this->db->where('state_sl<>',18);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
////////////////vesselownershipchange-----end-------///////////////
/////////////////vessel duplicate certificate----start-------//////////////


function get_vesseldupcertificate_List($user_id)       
{
     $this->db->select('*');   
   // $this->db->select('tbl_kiv_vessel_details.*');
    $this->db->select('tb_vessel_main.*');
...
function get_namechange_ref_number()
{
    $this->db->select('ref_number');
    $this->db->from('tbl_namechange');
    $this->db->where('ref_number<>','');
    $this->db->order_by('change_sl','DESC');
    $this->db->limit(1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_ownchange_ref_number()
{
    $this->db->select('ref_number');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('ref_number<>','');
    $this->db->where('transfer_changetype',2);
    $this->db->order_by('transfer_sl','DESC');
    $this->db->limit(1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_tnsfrvsl_rws($tsfr,$based)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('ref_number<>','');
    $this->db->where('transfer_changetype',$tsfr);
    $this->db->where('transfer_based_changetype',$based);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
}
function get_tnsfrvsl_ref_number($tsfr,$based)
{
    $this->db->select('ref_number');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('ref_number<>','');
...
function get_dupcert_ref_number()
{
    $this->db->select('ref_number');
    $this->db->from('tbl_duplicate_certificate');
    $this->db->where('ref_number<>','');
    $this->db->order_by('duplicate_cert_sl','DESC');
    $this->db->limit(1);
    $query  =   $this->db->get();
    $result =   $query->result_array();
    return $result;
...
function get_transfervessel_type($id)
{
    $this->db->select('*');
    $this->db->from('tbl_transfer_ownershipchange');
    $this->db->where('transfer_vessel_id',$id);
