...

{
...

	{
		...
		$res=$this->db->query("select user_master_id_user_type as utype from user_master where user_master_id='$uid'");
...
$this->db->where('lsg_section_status', 1);
	...
	function get_montlyPermit_by_periodname($period_name,$port_id){
		...
		$this->db->where('monthly_permit_permit_status!=3');
		$this->db->order_by('monthly_permit_permit_status','asc');
