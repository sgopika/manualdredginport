...
public function bookofregistration_list()
{
  ...
  $customer_id  = $this->session->userdata('customer_id');
  $survey_user_id = $this->session->userdata('survey_user_id');
  if(!empty($sess_usr_id) && (($user_type_id==3) || ($user_type_id==11) ))
  { 
      ...
      if($result_insert)
      {
       ...
       /* if(!empty($online_payment_data))
        ...
        { 
          ...
          /*________________GET reference number start-renewal of registration certificate_________*/
          date_default_timezone_set("Asia/Kolkata");
          $date         =   date('Y-m-d h:i:s', time());
          $process_id=10;
          ...
          $update_ref_number    =   $this->Survey_model->update_kiv_reference_number('tbl_kiv_reference_number',$data_ref_number, $ref_id);
          /*________________GET reference number end-renewal of registration certificate_____________*/
          /*__________________________________Processflow start_____________________________________*/
          ...
          $data_portofregistry =  array('vessel_registry_port_id' => $portofregistry_sl);
          $update_portofregistry    = $this->Survey_model->update_tbl_vessel_details('tbl_kiv_vessel_details',$data_portofregistry, $vessel_id);
          $port_registry_user_id           =   $this->Survey_model->get_port_registry_user_id($portofregistry_sl);
          $data['port_registry_user_id']  =   $port_registry_user_id;
          ...
          $data_renewal=array('registration_renewal_payment_date'=>$newDate,'payment_status'=>'1');
          ...
          if($process_id==42)
          {
            ...
            if($tariff_amount>0 && $portofregistry_sl!=false)
            {
              ...
              $process_insert=$this->Survey_model->insert_processflow('tbl_kiv_processflow', $data_insert);
              $status_update=$this->Survey_model->update_status_details('tbl_kiv_status_details', $data_survey_status,$status_details_sl);
              redirect('Kiv_Ctrl/Bookofregistration/renewal_certificate_list');
...
public function dataentry_reports()
{
  ...
  $customer_id       =  $this->session->userdata('customer_id');
  $survey_user_id    =  $this->session->userdata('survey_user_id');
  if(!empty($sess_usr_id) && (($user_type_id==14) || ($user_type_id==12) ))
  {
