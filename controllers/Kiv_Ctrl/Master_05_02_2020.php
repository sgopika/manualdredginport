...
/*---------------------------- checking of session set or not ----------------------------*/
	...
	{//echo "hh";exit;
		 ...
		 $sess_usr_id 	= 	$this->session->userdata('user_sl');*///print_r($_SESSION);
		 $sess_usr_id 	  = $this->session->userdata('int_userid');
  		 $int_usertype	  =	$this->session->userdata('int_usertype');
		 //if(!empty($sess_usr_id)&&($sess_usr_id==1))//
		 if(!empty($sess_usr_id)&&($sess_usr_id==1))
		 {
//echo "hiii";exit;
...
///////////////////Edit Vessel Category Start//////////////////////////
	...
	{
		//$sess_usr_id 				= 	$this->session->userdata('int_userpost_sl');
		//$sess_usr_id 			= 	1;
		$ip						=	$_SERVER['REMOTE_ADDR'];
		date_default_timezone_set("Asia/Kolkata");
		$date 					= 	date('Y-m-d h:i:s', time());
	...
	public function vessel_subcategory()
    {
		...
		{	
			...
			$data['vessel_category']	=	 $vessel_category;
                       ...
                       /* $vessel_category			=$this->Master_model->get_vessel_categoryname();
			$data['vessel_category']	=	 $vessel_category;
                        $data 					= 	$data + $this->data;*/
  ...
  ///////////////////Status Vessel Sub Category Start//////////////////////////
	public function status_vessel_subcategory()
	{
		...
		{	
			...
			$data 				= 	array(
				'vessel_subcategory_status' => $updstat
			);
...
$chkduplication		=	$this->Master_model->check_duplication_vessel_subcategory_edit($edit_vessel_subcategory_category_id,$edit_vessel_subcategory,$edit_vessel_subcategory_code,$id);
				...
				if($cntrows==0){
					...
					$data 			= 	array(
                                                        'vessel_subcategory_category_id'        =>      $edit_vessel_subcategory_category_id,
...
/*End*/
				...
				if($cntrows==0){
					...
					$data 			= 	array(
                                                        'vessel_subtype_vesseltype_id'      =>  $edit_vessel_subtype_vesseltype_id,
...

{
	...
	{	
		...
		$this->load->view('Kiv_views/Master/nozzle_type',$data);
...
public function add_locationof_electricalgenerator()
	{
		...
		$chkduplication=$this->Master_model->check_duplication_locationof_electricalgenerator_insert($locationof_electricalgenerator_name,$locationof_electricalgenerator_code); 
				...
				if($cntrows==0){
					...
					{
					$data 			= 	array(
					'locationof_electricalgenerator_name' 		=>	$locationof_electricalgenerator_name,  
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'natureofoperation_name' 		 =>	$edit_natureofoperation,  
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'firepumptype_name' 		 =>	$edit_firepumptype,  
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 			= 	array(
                                                        'document_type_id'              =>      $edit_document_type_id,
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'pollution_controldevice_name' 		 =>	$edit_pollution_controldevice,  
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'plyingstate_name' 		 =>	$edit_plyingstate,  
...
/*End*/	
				...
				$data 		= 	array(
				'fireextinguisher_subtype_name' 		 =>	$edit_fireextinguisher_subtype,  
				...
				if($cntrows==0){
					if($equipment_ins=="Save Equipment")
					{
						...
						if(isset($table_name))
						{
							...
							$tab_mal_name=$tab_det_val."_mal_name";
							$tab_code=$tab_det_val."_code";
							$tab_status=$tab_det_val."_status";
							$tab_created_user_id=$tab_det_val."_created_user_id";
							$tab_created_timestamp=$tab_det_val."_created_timestamp";
							$tab_created_ipaddress=$tab_det_val."_created_ipaddress";
							

							$tab_data = 	array(
							$tab_sl 		       =>	$insert_id,  
							$tab_name 		       => 	$equipment_name,  
							$tab_mal_name 	       => 	$equipment_mal_name,
							$tab_code 		       => 	$equipment_code,
							$tab_status			   =>	'1',
							$tab_created_user_id   => 	$sess_usr_id,
							$tab_created_timestamp =>	$date,
							$tab_created_ipaddress =>	$ip
						);
						$data = $this->security->xss_clean($tab_data);
						//insert the form data into corresponding master table
						$usr_res_master =	$this->db->insert($table_name, $data);
					}*/
							/*----------------*/
						echo json_encode(array("val_errors" => ""));
...
/*End*/	
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'user_type_type_name' 		 =>	$edit_user_type
							/*'user_type_modified_user_id'	 =>	$sess_usr_id,
							'user_type_modified_ipaddress'=>	$ip*/
						);
		...
		{	
			...
			if ($this->form_validation->run() == FALSE)
            {
				...
				if($cntrows==0){
					...
					{
						$data 			= 	array(
							'searchlight_size_name' 		=>	$searchlight_size_name,  
		...
		{	
			...
			if ($this->form_validation->run() == FALSE)
            {
				...
				if($cntrows==0){
					...
					$data 		= 	array(
							'searchlight_size_name' 		 =>	$edit_searchlight_size,  
		...
		{	
			...
			if ($this->form_validation->run() == FALSE)
            {
				...
				if($cntrows==0){
					...
					{
						$data 			= 	array(
							'mastertable_name' 		=>	$mastertable_name,  
...
//_____________________________________________________________________________________________________________________________________________
    ...
    function  mastertable_print()
	{
		...
		//$tableid 	=	$mastertable[0]['mastertable_sl'];
...

{
	...
	{	
       ...
       /* $this->load->view('template/header.php');
		...
		$this->load->view('Kiv_views/Master/dynamic_form_list',$data);
	...
	{		
		...
		{	
			...
			{ 
				...
				//$start_date 				= 	$this->input->post('start_date');commented on (06-07-2019)
				//$end_date 					= 	$this->input->post('end_date');commented on(06-07-2019)
				echo $start_date 				= 	$this->input->post('start_date_hidden');print_r($start_date);
				echo $end_date 					= 	$this->input->post('end_date_hidden');print_r($end_date);
				$label_id 					= 	$this->input->post('lab');//print_r($_REQUEST);exit;
				//$lab_values 				= 	$this->input->post('lab_values');print_r($lab_values);exit;
				...
				{ //echo "1-";echo $db_end_date;echo "cntrows>0";echo $start_date;exit;
				...
				{//echo "3-";echo "($cntrows>0)&&($start_date > $db_end_date)&&($start_date > $db_start_date))||($cntrows==0)";exit;
					/*Insert only selected lables to the dynamic table 
					foreach ($label_id as $labelid) {						
					
					if(!empty($labelid))
					{
											
						//$labelid=$this->security->xss_clean($labelid);
						$id=trim($labelid);
						$labvalues=$_REQUEST['lab_values_'.$id];
						if(!empty($labvalues)){echo $label_value_status=2;}else{echo $label_value_status=1;}
						$insertData = array();
						$tempArray = array(
						...
						//$usr_res	 = $this->db->insert('kiv_dynamic_field_master', $insertData);
					...
					$labelss					= 	$this->Master_model->get_label_dynamic($form_id,$heading_sl);
	           		...
	           		{          				
						...
						$insertData  = $this->security->xss_clean($insertData);//print_r($insertData);//exit;
						$usr_res	 = $this->db->insert_batch('kiv_dynamic_field_master', $insertData);					
						//$usr_res	 = $this->db->insert('kiv_dynamic_field_master', $insertData);		
					}
						...
						/*if($usr_res)
						{
							...
							{			
								...
								if(!empty($labelid))
								{	
									echo $id=trim($labelid);echo "<br>";
									echo "lab".$labvalues=$_REQUEST['lab_values_'.$id];echo "<br>";
									
									if(!empty($labvalues))
										{
											echo "sta2=".$label_value_status=2;echo "<br>";
										}
									if(empty($labvalues))
										{
											echo "sta1=".$label_value_status=1;echo "<br>";
										}								
								}		
								...
								$this->db->update('kiv_dynamic_field_master', $updateData);
							}
						}*/					
						/*_____________________________________________________________________________________________________________*/		
...
function dynamic_labels_ajax()
   { 	   
           ...
           $this->load->model('Kiv_models/Master_model');
           /*For counting total headings*/
           $heading_name			= 	$this->Master_model->get_heading_dynamic($form_id);
           $data['heading_name']	=	$heading_name;
           $countAllHeading 		=	count($heading_name);
           $data 					= 	$data + $this->data;

           $headingList				= 	$this->Master_model->get_dynamicForm_headingList($vesseltype_name,$vessel_subtype_name_opt,$length_over_deck,$hullmaterial_name,$engine_inboard_outboard,$form_id,$start_date,$end_date);
           $data['headingList']		=	$headingList; //print_r($headingList);
           ...
           echo '<tr><td colspan="4">Already Existing Headings Under the Selected Conditions</td></tr>';
           echo '<tr><td>Sl.No</td><td>Vessel Type Name</td><td>Vessel Sub Type Name</td><td>Hull Material</td><td>Lengtd Over the Deck</td><td>Inboard/ Outboard</td><td>Form Name</td><td>Heading Name</td><td>Date</td></tr>';
           $i=1;
           foreach ($headingList as $headingValue) 
           {   
           	   $start_date_view  = $headingValue['start_date'];
           	   $start_date_view  = explode('-', $start_date_view);
               $start_date_view  = $start_date_view[2]."/".$start_date_view[1]."/".$start_date_view[0];

               $end_date_view  = $headingValue['end_date'];
               $end_date_view  = explode('-', $end_date_view);
               $end_date_view  = $end_date_view[2]."/".$end_date_view[1]."/".$end_date_view[0];  


           
           	echo '<tr><td>'.$i.'</td><td>'.$headingValue['vesseltype_name'].'</td><td>'.$headingValue['vessel_subtype_name'].'</td>
            <td>';

            	if($headingValue['hullmaterial_id']==9999){echo "All";}else{echo $headingValue['hullmaterial_name'];} 

            echo '</td><td>'.$headingValue['length_over_deck'].'</td><td>';

            	if($headingValue['engine_inboard_outboard']==9999){echo "All";}else{echo $headingValue['inboard_outboard_name'];} 

            echo '</td><td>'.$headingValue['document_type_name'].'</td><td>'.$headingValue['heading_name'].'</td>';

            	if($headingValue['end_date']=='0000-00-00'){$msg="dd/mm/yyyy";}else{$msg=$end_date_view;}

            echo '<td> From '.$start_date_view.' To '.$msg.'</td></tr>';

            echo '<div class="alert alert-primary" role="alert">'.$headingValue['heading_name'].'</div>'; 
            $i++;
           }
           echo '<table>';*/
           ...
           /*If the selected heading already existing, not need to display the labels under that heading*/		 
			...
			{ 
				...
				//echo '<table class="table table-bordered table-striped"><tr><td><input type="checkbox" name="lab[]" id="lab_'.$label_sl.'" value="'.$label_sl.' " onclick="checkedVal('.$label_sl.')"></td><td>'.$label_name.'</td><td>Value</td><td><input type="text" name="lab_values_'.$label_sl.'" id="lab_values_'.$label_sl.'" onclick="checkboxVal('.$label_sl.')" maxlength="50" class="form-control" placeholder=" Enter Values" autocomplete="off"></td></tr><table>';
...
*/
			/*$dynamic_form = array(
							    //'dynamic_field_sl'	=>	$edit_id,
								'heading_id'		=>	$edit_id,
								'vesseltype_id'		=>  $vesseltype_id,
								'vessel_subtype_id' =>  $vess_sub_id,
								'length_over_deck'	=> 	$length_over_deck,
								'hullmaterial_id'	=>	$hullmaterial_id,
								'engine_inboard_outboard0,
								'start_date'	=>	$start_date,
								'end_date'	=>	$end_date
							);*/
						//redirect('Kiv_Ctrl/Master/dynamic_form');
					...
					//redirect('Kiv_Ctrl/Master/detListViewDynamicform_byvesselform/$dynamic_form');
				}
				}
			}
		 /* else 
			{
				echo json_encode(array("val_errors" => "Dynamic Form Name/Malayalam Name/Code Already Exists!!!"));
...
{ 
	...
	/*echo $surveyName=validate_segment('3', 'required');exit;
	$this->form_validation->set_rules('surveyName', 'Survey Activity Name', 'required');
	$this->form_validation->set_rules('formtypeName', 'Form Name', 'required');
	$this->form_validation->set_rules('vesseltype_name', 'Vessel Type Name', 'required');
    ...
    {         
		$this->session->set_flashdata('msg','<div class="alert alert-success text-center">'. validation_errors().'</div>');
		//redirect('Kiv_Ctrl/Master/addTariff');
		//echo "error occurs";
	...
	{
	echo $surveyName 			= 	$this->input->post('surveyName');
	echo $formtypeName			= 	$this->input->post('formtypeName');
	echo $vesseltype_name 		= 	$this->input->post('vesseltype_name');
	echo $vessel_subtype_name 	= 	$this->input->post('vessel_subtype_name');	
	echo $startDate 			= 	$this->input->post('startDate');
	echo $endDate 				= 	$this->input->post('endDate');exit;*/
...
{	
	...
	$formtypeName			= 	$this->input->post('formtypeName');
	$vesseltype_name 		= 	$this->input->post('vesseltype_name');
	$vessel_subtype_name 	= 	$this->input->post('vessel_subtype_name');	
	...
	{	//echo "hiii";exit;
		...
		if ($this->form_validation->run() == FALSE)
        {
			...
			if($cntrows>0){
				...
				$data 				= 	array(
					'webnotification_engtitle' 			=>	$title_eng,
		...
		if ($this->form_validation->run() == FALSE)
        {
			...
			if($cntrows>0){
				...
				$data 				= 	array(
					...
					'bodycontent_module_sl'			=>	$moduleid
				); //print_r($data);exit;
				$data = $this->security->xss_clean($data);
				//insert the form data into database
