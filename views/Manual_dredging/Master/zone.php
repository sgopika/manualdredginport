...
function del_zone(zone_id)
{
	...
	{
	$.ajax({
				url : "<?php echo site_url('Manual_dredging/Port/delzone/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
				success: function(data)
...
<div class="row py-1">
	...
	<div class="col-8 d-flex justify-content-end">
        ...
        <li class="breadcrumb-item"><a href="<?php echo site_url("Manual_dredging/Master/pcdredginghome"); ?>"> Home</a></li>
          <li class="breadcrumb-item"><a href="#"><strong>Zone</strong></a></li>
      </ol>
</nav>
</div> <!-- end of col-8 -->
 ...
 <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
	...
	<div class="row py-3" id="view_vesselcategory">
		<div class="col-12">
            <a href="<?php echo $site_url.'/Manual_dredging/Master/zone_add';?>">
             <button type="button" class="btn btn-sm btn-primary" > <i class="fa fa-plus-circle"></i>&nbsp; Add New Zone </button>
              </a>
         </div> <!-- end of col12 -->
    </div> <!-- end of view row -->
    ...
    <div class="row">
		...
		<table id="example" class="table table-bordered table-striped table-hover">
				...
				//print_r($data);
				$allegation=array();
				 $i=1; foreach($zone as $rowmodule){
					 $id = $rowmodule['zone_id'];
					?>
					<tr id="<?php echo $i;?>">
                        ...
                        <td id="col_div_<?php echo $i; ?>">
                        <div id="first_<?php echo $i;?>"><?php echo strtoupper($rowmodule['zone_code']);?></div>
                         <div id="hide_<?php echo $i;?>" style="display:none"><input type="text" name="edit_allegation_<?php echo $i;?>"  id="edit_allegation_<?php echo $i;?>" value="<?php echo $rowmodule['zone_code'];?>" onchange="check_dup_allegation_edit(<?php echo $i;?>);"   maxlength="100" autocomplete="off"/>
                        </div>
                        </td>
						<?php 
							if ($rowmodule['zone_status']==1) {
								?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['zone_status'];?>);"  > <i class="fa fa-fw  fa-check-circle-o"></i> &nbsp;&nbsp; Active &nbsp;&nbsp; </button> </td>
								<?php
							}
							else {
								?>
								<td> <button class="btn btn-sm btn-info btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['zone_status'];?>);"> <i class="fa fa-fw  fa-minus"></i> &nbsp; Inactive  </button> </td>
								<?php
								// error_reporting(E_ALL);
							} ?>
						<td id="td_div_<?php echo $i;?>"> 
                        <div id="edit_div_<?php echo $i;?>">
                        <a href="<?php echo site_url('Manual_dredging/Master/zone_edit/'.encode_url($id));?>">
                        <button name="edit_allegation_btn_<?php echo $i;?>" id="edit_allegation_btn_<?php echo $i;?>" class="btn btn-sm bg-purple btn-flat" type="button" <?php if ($rowmodule['zone_status']==0) {?> disabled="disabled" <?php } ?>>   <i class="fa fa-fw  fa-pencil"></i> &nbsp; Edit  </button> </a>						
                        </div>
               
                        </td>
						<td> <button class="btn btn-sm btn-danger btn-flat" type="button" onclick="del_zone(<?php echo $rowmodule['zone_id']; ?>)" <?php if ($rowmodule['zone_status']==0) {?> disabled="disabled" <?php } ?>> <i class="fa fa-fw fa-trash"></i> &nbsp; Delete  </button> </td>
					</tr>
					<?php
					$i++; 
				}
