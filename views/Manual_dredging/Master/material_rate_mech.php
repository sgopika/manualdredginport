...
function del_matrate(zone_id)
{
	...
	{
	$.ajax({
				url : "<?php echo site_url('Port/delmatrate_mech/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
      ...
      <div class="row custom-inner">
		...
		if(isset($editres)){
            ...
            <div class="box-header" id="view_allegation">
               <a href="<?php echo $site_url.'/Port/add_mech_matrate';?>"> <button class="btn btn-primary btn-flat" type="button" > <i class="fa fa-fw fa-plus-circle"></i> &nbsp;&nbsp;Add Material Rate</button>
              </a>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
				...
				$allegation=array();
				 $i=1; foreach($material_rate as $rowmodule){
					...
					<tr id="<?php echo $i;?>">
                       ...
                       <a href="<?php echo $site_url.'/Port/edit_mechmaterialrate/'.encode_url($id);?>"> <button <?php if ($rowmodule['mdrate_status']==2) { ?> disabled="disabled" <?php }?> name="edit_allegation_btn_<?php echo $i;?>" id="edit_allegation_btn_<?php echo $i;?>" class="btn btn-sm bg-purple btn-flat" type="button"  ><i class="fa fa-fw  fa-pencil"></i> &nbsp; Edit  </button> 	</a>					
                        </div>
