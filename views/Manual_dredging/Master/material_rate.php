...
function del_matrate(zone_id)
{
	...
	{
	$.ajax({
				url : "<?php echo site_url('Manual_dredging/Port/delmatrate/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
...
<div class="row py-1">
	...
	<div class="col-8 d-flex justify-content-end">
     ...
     <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
		...
		<div class="col-12">
               <a href="<?php echo $site_url.'/Manual_dredging/Master/materialrate_add';?>"> <button class="btn btn-primary btn-flat" type="button" > <i class="fa fa-fw fa-plus-circle"></i> &nbsp;&nbsp;Add Material Rate</button>
              </a>
            </div></div>
            <!-- /.box-header -->
             <div class="row">
		 ...
		 <table id="example" class="table table-bordered table-striped">
				...
				$allegation=array();
				 $i=1; foreach($material_rate as $rowmodule){
					 if($rowmodule['materialrate_port_user_id']==$user)
					 {
					...
					<tr id="<?php echo $i;?>">
                        ...
                        <a href="<?php echo $site_url.'/Manual_dredging/Master/materiaratel_master_edit/'.encode_url($id);?>"> <button name="edit_allegation_btn_<?php echo $i;?>" id="edit_allegation_btn_<?php echo $i;?>" class="btn btn-sm bg-purple btn-flat" type="button" <?php if($rowmodule['materialrate_port_status']==0){ ?> disabled="disabled" <?php } ?>  ><i class="fa fa-fw  fa-pencil"></i> &nbsp; Edit  </button> 	</a>					
                        </div>
                        </td>
						<td><?php if($rowmodule['materialrate_port_end_date']!='0000-00-00'){ ?> <button class="btn btn-sm btn-danger btn-flat" type="button" onclick="del_matrate(<?php echo $rowmodule['materialrate_port_id']; ?>)" <?php if($rowmodule['materialrate_port_status']==0){ ?> disabled="disabled" <?php } ?>> <i class="fa fa-fw fa-trash"></i> &nbsp; Delete  </button><?php  } else { echo "Update End Date to Delete data"; }?></td>
					</tr>
					<?php
					$i++; 
				}
