...
function del_taxname(zone_id)
{
	var a=confirm("are you sure?");
	if(a==true)
	{
	$.ajax({
				url : "<?php echo site_url('Manual_dredging/Port/deltax/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
...
<div class="row py-1">
	...
	<div class="col-8 d-flex justify-content-end">
        ...
        <li class="breadcrumb-item"><a href="#"><strong>Tax Name</strong></a></li>
      </ol>
</div> <!-- end of col-8 -->
     ...
     <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
        ...
        <?php
		$flg=0;
		foreach($taxname as $rowmodule)
		{
			if($rowmodule['taxname_master_status']==1)
			{
				$flg=1;
			}
		}
		...
		<div class="col-12">
           <!--<a href="<?php echo $site_url.'Manual_dredging/Master/taxnamemaster_add';?>"> <button class="btn btn-primary btn-flat" type="button" <?php if($flg==1){?> disabled="disabled" <?php }?> > <i class="fa fa-fw fa-plus-circle"></i> &nbsp;&nbsp;Add New Tax </button>
              </a>-->
            <div class="row">
		<div class="col-12">
		<!-- --------------------------------------------- start of table column ------------------------------------- -->
		 <table id="example" class="table table-bordered table-striped">
				...
				//print_r($data);
				$allegation=array();
				 $i=1; foreach($taxname as $rowmodule){
					 $id = $rowmodule['taxname_master_id'];
					?>
					<tr id="<?php echo $i;?>">
                        ...
                        <?php 
							if ($rowmodule['taxname_master_status']==1) {
								?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['taxname_master_status'];?>);"  > <i class="fa fa-fw  fa-check-circle-o"></i> &nbsp;&nbsp; Active &nbsp;&nbsp; </button> </td>
								<?php
							}
							else {
								?>
								<td> <button class="btn btn-sm btn-info btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['taxname_master_status'];?>);"> <i class="fa fa-fw  fa-minus"></i> &nbsp; Inactive  </button> </td>
								<?php
							} ?>
						<td id="td_div_<?php echo $i;?>"> 
                        <div id="edit_div_<?php echo $i;?>">
                        <a href="<?php echo $site_url.'/Manual_dredging/Master/taxname_master_edit/'.encode_url($id);?>">
						...
						<td> <button class="btn btn-sm btn-danger btn-flat" type="button" onclick="del_taxname(<?php echo $rowmodule['taxname_master_id'];?>)"> <i class="fa fa-fw fa-trash"></i> &nbsp; Delete  </button> </td>
					</tr>
					<?php
					$i++; 
				}
