...
function del_taxcalc(zone_id)
{
	var a=confirm("are you sure?");
	if(a==true)
	{
	$.ajax({
				url : "<?php echo site_url('Manual_dredging/Port/deltaxcalc/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
...
<div class="row py-1">
	...
	<div class="col-8 d-flex justify-content-end">
        ...
        <li class="breadcrumb-item"><a href="#"><strong>Tax Calculator</strong></a></li>
      </ol>
</div> <!-- end of col-8 -->
     ...
     <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
		...
		$flg=0;
		foreach($tax_rate as $rowmodules)
		{
			if($rowmodules['tax_calculator_status']==1)
			{
				$flg=1;
			}
		}
		?><?php
			$stat_f=0;
			foreach($tax_rate as $rowmodule){
				if ($rowmodule['tax_calculator_status']==1)
					{
						$stat_f=1;
					}	
				}
		...
		<div class="col-12">
               <a href="<?php echo $site_url.'/Manual_dredging/Master/taxcalculator_add';?>"> <button class="btn btn-primary btn-flat" <?php if($stat_f==1){?> disabled="disabled" <?php } ?>  type="button"> <i class="fa fa-fw fa-plus-circle"></i> &nbsp;&nbsp;Add New</button>
              </a>
            </div></div>
            <div class="row">
		<div class="col-12">
		<!-- --------------------------------------------- start of table column ------------------------------------- -->
		 <table id="example" class="table table-bordered table-striped">
                ...
                <tr>
                  <th id="sl">Sl.No</th>
                  <th id="col_name">Tax Name</th>
                  <th id="col_name">Tax Rate</th>
                  <th>Status</th>
				...
				//print_r($data);
				$allegation=array();
				 $i=1; foreach($tax_rate as $rowmodule){
					 $id = $rowmodule['tax_calculator_id'];
					?>
					<tr id="<?php echo $i;?>">
						<td  id="sl_div_<?php echo $i; ?>"> <?php echo $i; ?> </td>
						<td id="col_div_<?php echo $i; ?>">
                        <div id="first_<?php echo $i;?>"><?php echo strtoupper($rowmodule['taxname_master_name']);?></div>
                         <div id="hide_<?php echo $i;?>" style="display:none"><input type="text" name="edit_allegation_<?php echo $i;?>"  id="edit_allegation_<?php echo $i;?>" value="<?php echo $rowmodule['taxname_master_name'];?>" onchange="check_dup_allegation_edit(<?php echo $i;?>);"   maxlength="100" autocomplete="off"/>
                        </div>
                        </td>
                        <td id="col_div_<?php echo $i; ?>">
                        <div id="first_<?php echo $i;?>"><?php echo strtoupper($rowmodule['tax_calculator_rate']);?></div>
                         <div id="hide_<?php echo $i;?>" style="display:none"><input type="text" name="edit_allegation_<?php echo $i;?>"  id="edit_allegation_<?php echo $i;?>" value="<?php echo $rowmodule['tax_calculator_rate'];?>" onchange="check_dup_allegation_edit(<?php echo $i;?>);"   maxlength="100" autocomplete="off"/>
                        </div>
                        </td>
						<?php 
							if ($rowmodule['tax_calculator_status']==1) {
								?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['tax_calculator_status'];?>);"  > <i class="fa fa-fw  fa-check-circle-o"></i> &nbsp;&nbsp; Active &nbsp;&nbsp; </button> </td>
								<?php
							}
							else {
								?>
								<td> <button class="btn btn-sm btn-info btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['tax_calculator_status'];?>);"> <i class="fa fa-fw  fa-minus"></i> &nbsp; Inactive  </button> </td>
								<?php
							} ?>
						<td id="td_div_<?php echo $i;?>"> 
                        <div id="edit_div_<?php echo $i;?>">
                        <a href="<?php echo $site_url.'/Manual_dredging/Master/taxcalculator_edit/'.encode_url($id);?>"><button name="edit_allegation_btn_<?php echo $i;?>" id="edit_allegation_btn_<?php echo $i;?>" class="btn btn-sm bg-purple btn-flat" type="button" <?php if($rowmodule['tax_calculator_status']==0){?> disabled="disabled" <?php } ?>>   <i class="fa fa-fw  fa-pencil"></i> &nbsp; Edit  </button> </a>						
                        </div>
                        </td>
						<td><?php if($rowmodule['tax_calculator_end_date']!='0000-00-00'){ ?> <button class="btn btn-sm btn-danger btn-flat" type="button" onclick="del_taxcalc(<?php echo $id;?>)" <?php if($rowmodule['tax_calculator_status']==0){?> disabled="disabled" <?php } ?>> <i class="fa fa-fw fa-trash"></i> &nbsp; Delete  </button><?php } else { echo "Update End Date to Delete data"; }?> </td>
					</tr>
					<?php
					$i++; 
				}
