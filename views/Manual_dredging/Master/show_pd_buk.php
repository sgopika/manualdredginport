...
<div class="row py-1">
	<div class="col-4 breaddiv">
		<span class="badge bg-darkmagenta innertitle mt-2">Customer Booking Details</span>
	</div>  <!-- end of col4 -->
	
	<div class="col-8 d-flex justify-content-end">
		<ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?php echo site_url("Manual_dredging/Port/port_dir_main"); ?>"> Home</a></li>
        <li class="breadcrumb-item"><a href="#"><strong>Customer Booking Details</strong></a></li>
      </ol>
</div> <!-- end of col-8 -->
     ...
     <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
		...
		<div class="col-12">
               <a href="<?php echo site_url("Manual_dredging/Port/cus_reg_det_pd");?>"> <button class="btn btn-primary btn-flat" type="button" > <i class="fa fa-fw fa-arrow-left"></i> &nbsp;&nbsp;Go Back </button>
              </a>
            </div>
			</div>
...
<tr>
    ...
    <th>Booking Date</th>
    <th>Sand Needed Date</th>
    <th>Status</th>
</tr>
<?php
foreach($buk_det as $b_d)
{
?>
<tr>
	<td><?php echo $i; ?></td>
    <td><?php echo $b_d['customer_name'];?></td>
    <td><?php echo $b_d['zone_name'];?></td>
    <td><?php echo $b_d['customer_booking_token_number'];?></td>
    <td><?php echo $b_d['customer_aadhar_number'];?></td>
    <td><?php echo $b_d['customer_booking_request_ton'];?></td>
    <td><?php echo  date("d/m/Y h:i:s", strtotime(str_replace('-', '/',$b_d['customer_booking_requested_timestamp'] )));?></td>
    <td><?php echo date("d/m/Y h:i:s", strtotime(str_replace('-', '/',$b_d['customer_booking_allotted_date'] )));?></td>
    <?php 
							if ($b_d['customer_booking_decission_status']==0) {
								?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button"> <i class="fa fa-fw  fa-minus"></i> &nbsp;&nbsp; Waiting For Approval &nbsp;&nbsp; </button> </td>
								<?php
							}
							else if($b_d['customer_booking_decission_status']==2)
							{
								$curd=date('Y-m-d');
								if(($b_d['customer_booking_allotted_date']<$curd)&&($b_d['customer_booking_pass_issue_user']==''))
								{
									$sat=1;
									?>
                                    <td> <button class="btn btn-sm btn-info btn-flat" type="button"> <i class="fa fa-fw  fa-minus"></i> &nbsp; Sand Not Taken </button> </td>
                                    <?php
								}
								else if($b_d['customer_booking_pass_issue_user']!=0)
								{
									$sat=1;
									?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button"> <i class="fa fa-fw  fa-check-circle-o"></i> &nbsp; Sand Issued </button> </td>
								<?php
								}
								else
								{
