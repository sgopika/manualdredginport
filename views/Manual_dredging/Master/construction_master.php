...
function del_const(zone_id)
{
	...
	{
	$.ajax({
				url : "<?php echo site_url('Manual_dredging/Port/delconstr/')?>",
				type: "POST",
				data:{id:zone_id},
				dataType: "JSON",
...
<div class="row py-1">
	...
	<div class="col-8 d-flex justify-content-end">
        ...
        <li class="breadcrumb-item"><a href="#"><strong>Construction</strong></a></li>
      </ol>
</div> <!-- end of col-8 -->
     ...
     <div id="msgDiv" class="alert alert-info alert-dismissible" style="display:none"></div> 
		...
		<div class="col-12">
               <a href="<?php echo $site_url.'/Manual_dredging/Master/construction_add';?>"> <button class="btn btn-primary btn-flat" type="button" > <i class="fa fa-fw fa-plus-circle"></i> &nbsp;&nbsp;Add New</button>
              </a>
            </div></div>
            <div class="row">
		<div class="col-12">
		<!-- --------------------------------------------- start of table column ------------------------------------- -->
		 <table id="example" class="table table-bordered table-striped">
                ...
                <tr>
                  <th id="sl">Sl.No</th>
                  <th id="col_name">Construction Name</th>
                  <th>Status</th>
				...
				$allegation=array();
				 $i=1; foreach($cnst as $rowmodule){
					...
					<tr id="<?php echo $i;?>">
                        ...
                        <div id="first_<?php echo $i;?>"><?php echo strtoupper($rowmodule['construction_master_name']);?></div>
                         <div id="hide_<?php echo $i;?>" style="display:none"><input type="text" name="edit_allegation_<?php echo $i;?>"  id="edit_allegation_<?php echo $i;?>" value="<?php echo $rowmodule['construction_master_name'];?>" onchange="check_dup_allegation_edit(<?php echo $i;?>);"   maxlength="100" autocomplete="off"/>
                        </div>
                        </td>
                        <?php 
							if ($rowmodule['construction_master_status']==1) {
								?>
								<td> <button class="btn btn-sm btn-success btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['construction_master_status'];?>);"  > <i class="fa fa-fw  fa-check-circle-o"></i> &nbsp;&nbsp; Active &nbsp;&nbsp; </button> </td>
								<?php
							}
							else {
								?>
								<td> <button class="btn btn-sm btn-info btn-flat" type="button" onclick="toggle_status(<?php echo $id;?>,<?php echo $rowmodule['construction_master_status'];?>);"> <i class="fa fa-fw  fa-minus"></i> &nbsp; Inactive  </button> </td>
								<?php
							} ?>
						<td id="td_div_<?php echo $i;?>"> 
                        <div id="edit_div_<?php echo $i;?>">
                        <a href="<?php echo $site_url.'/Manual_dredging/Master/construction_master_edit/'.encode_url($id);?>">
						...
						<td> <button class="btn btn-sm btn-danger btn-flat" type="button" onclick="del_const(<?php echo $id;?>)"> <i class="fa fa-fw fa-trash"></i> &nbsp; Delete  </button> </td>
					</tr>
					<?php
					$i++; 
				}
