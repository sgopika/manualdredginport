   ...
   <div class="container-fluid ui-innerpage">
      ...
      <!--      </div> -->
        ...
        <div class="box box-primary box-green-bottom">
			...
			//print($get_bookingapprovedadd);

			foreach($get_bookingapprovedadd as $messages)

			{

			$customerbookingid=$messages['spotreg_id'];

			$cutomername=$messages['spot_cusname'];

			$uidnumber=$messages['uid_no'];

			$tokenno=$messages['spot_token'];

			$challanno=$messages['spot_challan'];

	

			$challandate=date("d/m/Y h:i:s", strtotime(str_replace('-', '/',$messages['challan_timestamp'] )));

			$challanamount=$messages['spot_amount'];
			...
			$spot_booking_type=$messages['spot_booking_type'];
			

			}

			if($spot_booking_type==1){
			$url=$site_url.'/Manual_dredging/Report/sand_issue_add/'.encode_url($customerbookingid);
			}
			else
			{
				$url=$site_url.'/Manual_dredging/Report/sand_issue_adddoor_nw/'.encode_url($customerbookingid);
			}
			 ...
			 <tr><td style="border:1px solid #0000CC;" colspan="4"><table style="margin:10px" width="90%">
			  ...
			  <tr><td>Customer Name :</td><td><?php echo $cutomername ; ?></td><td>UID No:</td><td><?php echo $uidnumber; ?></td></tr>
				 ...
				 <tr>
				   ...
				   <tr>
				    ...
				    <tr>
					 ...
					 <tr><td colspan="4" align="right"> <?php if($transactionamount>=$challanamount){ ?> <a href="<?php echo $url;?>"><button class="btn btn-sm bg-blue btn-flat" type="button">  &nbsp; NEXT   </button></a><?php }else{ ?><font color="#CC0000"><?php echo "Transaction Amount is less than Challan Amount ";?> </font><?php } ?></td></tr>
