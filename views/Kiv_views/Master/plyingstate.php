...
function ins_plyingstate()
{
  ...
  if(plyingstate_name=="")
        ...
        $.ajax({
          url : "<?php echo site_url('Kiv_Ctrl/Master/add_plyingstate/')?>",
          type: "POST",
...
function toggle_status(id,status)
{
  ...
  var csrf_token    = '<?php echo $this->security->get_csrf_hash(); ?>';
  $.ajax({
        url : "<?php echo site_url('Kiv_Ctrl/Master/status_plyingstate/')?>",
        type: "POST",
  ...
  function del_plyingstate(id,status)
{
  var csrf_token    = '<?php echo $this->security->get_csrf_hash(); ?>';
  $.ajax({
        url : "<?php echo site_url('Kiv_Ctrl/Master/delete_plyingstate/')?>",
        type: "POST",
        data:{ id:id,stat:status,'csrf_test_name': csrf_token},
        dataType: "JSON",
        success: function(data)
...
function save_plyingstate(id,i)
{
  ...
  if(edit_plyingstate=="" ||  edit_plyingstate_code==""){
    ...
    $.ajax({
          url : "<?php echo site_url('Kiv_Ctrl/Master/edit_plyingstate/')?>",
          type: "POST",
...
<div class="row py-1">
  <div class="col-4 breaddiv">
    <span class="badge bg-darkmagenta innertitle "> Playing State </span>
  </div>  <!-- end of col4 -->
  <div class="col-8">
...
<!---------------------------------------- start of main content area  ---------------------------------------- -->
    ...
    if(isset($editres)){
          echo form_open("Kiv_Ctrl/Master/add_plyingstate", $attributes);
    } else {
      echo form_open("Kiv_Ctrl/Master/add_plyingstate", $attributes);
    }?>
    ...
    <div class="row py-3" id="add_plyingstate" style="display:none;">
             <div class="col-4">
             <input type="text" name="plyingstate_name" maxlength="20" id="plyingstate_name" class="form-control "  placeholder=" Enter Plying State Name" autocomplete="off"/>
             </div> <!-- end of col34 -->
              <div class="col-4">
