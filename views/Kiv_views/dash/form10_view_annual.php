...
$owner_user_type_id         =   $user_master[0]['user_master_id_user_type'];

}

$current_status1          =   $this->Survey_model->get_status_details($vessel_id,$survey_id1);
...
//----------Get Garbage : tbl_kiv_garbage_bucket_provider_details---------------//
 ...
 //----------Get next drydock details : tbl_kiv_vessel_timeline ---------------//
     ...
     {
      $timeline_sl=$annual_survey[0]['timeline_sl'];
...
<!-- <form name="form1" id="form1" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/Survey/form10_annual_entry/'.$vessel_id1.'/'.$processflow_sl1.'/'.$survey_id1 ?>">
 -->

 <?php
$attributes = array("class" => "form-horizontal", "id" => "form1", "name" => "form1", "enctype"=> "multipart/form-data");
  echo form_open("Kiv_Ctrl/Survey/form10_annual_entry/".$vessel_id1.'/'.$processflow_sl1.'/'.$survey_id1, $attributes);
  ?>
...
<div class="col-4 d-flex justify-content-left mt-1 mb-1 text-primary formfont" >
<?php if(!empty($natureofoperation_name)) { echo $natureofoperation_name; } else { echo $nil; } ?> 
</div> <!-- end of col-3 4th-->
</div> <!-- end of row -->
...
<!-- ___________________Submit button___________________________ -->
...
<button type="submit" class="btn btn-flat btn-sm btn-success btn-point" id="btnsubmit">Approve &nbsp; <i class="fas fa-arrow-right"></i> </button>    
</div> <!-- end of col-3 2nd-->
</div> <!-- end of row -->
