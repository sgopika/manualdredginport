...
<div class="col-12"> 
  <div class="row">
    <div class="col-4 mt-1 ml-5">
      <button type="button" class="btn btn-primary kivbutton btn-block"> Registered Vessel List For Ownership Change</button> 
    </div> <!-- end of col-2 --> 
  </div> <!-- inner row -->
...
<div class="main-content ui-innerpage">
  ...
  <!-- <table id="example" class="display mytableheader" style="width:100%" border="0"> -->
    ...
    <thead>
      <tr><th colspan="9"><font size="4">Ownership Change-Registered Vessel List</font></th></tr>
      <tr>
        <th>#</th>
        <th>Vessel Name</th>
        <th>Reg. No</th>
    ...
    { 
      ...
      $check_ownershipchange      = $this->Vessel_change_model->check_vessel_ownchg_pay($vessel_sl2,$vesselmain_sl); 
      ...
      if($processing_status==0){ 
        if(isset($pay_status)){if($pay_status==0){?>
          <a href="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/payment_details_form18/'.$vessel_sl1.'/'.$processflow_sl.'/'.$status_details_sl;?>" class="btn btn-danger btn-xs">Ownership Change Payment Pending</a>
        <?php } else if($pay_status==2){ ?> 
          <a href="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/ownershipchange/'.$vessel_sl1.'/'.$processflow_sl.'/'.$status_details_sl;?>" class="btn btn-danger btn-xs">Proceed to Ownership Change</a>
