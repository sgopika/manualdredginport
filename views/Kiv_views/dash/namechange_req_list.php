...
<!-- <table id="example" class="display mytableheader" style="width:100%" border="0"> -->
  ...
  <thead>
  <tr><th colspan="9"><font size="4">Name Change of Registered Vessels</font></th></tr>  
  <tr>
  <th>#</th>
  <th>Vessel Name</th>
  <th>Vessel Type</th>
 ...
 //print_r($namechange_det);
    foreach($namechange_det as $res1)
    {
      ...
      $data['status_det'] =  $status_det;//print_r($status_det);
      ...
      if(!empty($status_det)){ 
        foreach($status_det as $res_stat)
       {
        ...
        ////
        if($pay_status==0){
          $message='<span class="badge badge-success">Payment Pending</span>';
        $button_message='<span class="badge bg-success">Payment</span>';
        } else {
        ...
        foreach($approved_username as $res_appvname)
        {
        $appvd_usertyp       = $res_appvname['user_type_name'];
        }
       }
        $message='<span class="badge badge-success">APPROVED BY '.$appvd_usertyp.'</span>';
        $button_message='<span class="badge bg-success">VIEW</span>';
        }
      }*/
