...
$survey_intimation          = $this->Survey_model->get_survey_intimation($vessel_id,$survey_id);
    ...
    {
      foreach($survey_intimation as $sur_intim){
        $intimation_created_user_id=$sur_intim['intimation_created_user_id'];
      }
      
    }

if(!empty($intimation_created_user_id)){
$user_sl                 =   $this->Survey_model->get_user_sl($intimation_created_user_id);
$data['user_sl']         =   $user_sl;
if(!empty($user_sl))
{
  $user_sl_cs_sr=   $intimation_created_user_id;
  $user_type_id_cs_sr=   $user_sl[0]['user_master_id_user_type'];
} else {
  $user_sl_cs_sr=   0;
  $user_type_id_cs_sr=   0;
}
} else {
  $user_sl_cs_sr=   0;
  $user_type_id_cs_sr=   0;
...
<!-- ########################################################################################### -->
 <!-- inside content -->
<div><font size="4"><strong>Form 11</strong></font></div>

<form name="form11_payment" id="form11_payment" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/Verify_payment_pc_form11/'.$vessel_id1.'/'.$processflow_sl1.'/'.$survey_id1 ?>">
