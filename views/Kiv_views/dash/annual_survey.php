...
$current_status1          =   $this->Survey_model->get_status_details_vessel_sl($vessel_id);
...
//alert(cntcount);
 ...
 /*  for(var j=1; j<=9; j++)
  {
    if($('input[name="form2_myFile'+j+'"]').length)
    {
      var file_data = $('input[name="form2_myFile'+j+'"]').prop('files')[0];  
      //alert(file_data);
      if(file_data!='undefined')
      {
        data.append("form2_myFile"+j, file_data);
      }
    }
  }
  ...
  for(var k=10; k<=13; k++)
  {
    if($('input[name="form2_myFile'+k+'"]').length)
    {
      var file_data = $('input[name="form2_myFile'+k+'"]').prop('files')[0];  
      if(file_data!='undefined')
      {
        data.append("form2_myFile"+k, file_data);
      }
    }
  }*/

  var file_data = $('input[name="form2_myFile11"]').prop('files')[0];  
  if(file_data!='undefined')
  {
    data.append("form2_myFile11", file_data);
  }

  var file_data = $('input[name="form2_myFile12"]').prop('files')[0];  
  if(file_data!='undefined')
  {
    data.append("form2_myFile12", file_data);
  }

    $.ajax({
    url : "<?php echo site_url('Kiv_Ctrl/Survey/annual_survey_entry')?>",
    type: "POST",
    //dataType: "JSON",
...
{*/
    $.ajax({
      url : "<?php echo site_url('Kiv_Ctrl/Survey/form2_annual_survey')?>",
      type: "POST",
      ...
      success: function(data)
      {
        ...
        alert("form2 inserted Successfully");
...
/*$("#tab3next").click(function()
{
  ...
  { 
    $.ajax({
      url : "<?php echo site_url('Kiv_Ctrl/Survey/annual_survey_payment')?>",
      type: "POST",
...
<div class="col-2 mt-1 ml-5">
         <button type="button" class="btn btn-primary kivbutton btn-block"> Form 2</button> 
      </div> <!-- end of col-2 -->
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  <li class="nav-item">
    <a class="nav-link active" id="form2tab" data-toggle="tab" href="#tab1" role="tab" aria-controls="form2Details" aria-selected="true">Annual survey entry</a>
  </li>
   <li class="nav-item">
    <a class="nav-link" id="form2view" data-toggle="tab" href="#tab2" role="tab" aria-controls="form2_View " aria-selected="true">Form2</a>
  </li>
   <li class="nav-item">
    <a class="nav-link" id="form2fees" data-toggle="tab" href="#tab3" role="tab" aria-controls="form2payments" aria-selected="true">Fee details</a>
  </li>
  
</ul>

<!-- ______________________ form 2 Details Start _________________________ -->

<div class="tab-content " id="myTabContent">

<div class="tab-pane fade show active" id="tab1" role="tabpanel" aria-labelledby="form2tab">
<!-- start of content in tab pane -->

<!-- <form name="form1" id="form1" method="post" class="form1"  action="" enctype="multipart/form-data" >  -->

  <?php
$attributes = array("class" => "form-horizontal", "id" => "form1", "name" => "form1", "enctype"=> "multipart/form-data");
echo form_open("Kiv_Ctrl/Survey/annual_survey_entry", $attributes);
?>
...
<!-- <div class="row no-gutters eventab border-bottom" id="survey1">
<div class="col-12 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1">
Documents as per rules-6 (3) [ (a) to ( f) ]
</div>
</div> -->
<?php
 /* $i=1; 
  if(!empty($list_document_vessel)) {
  foreach($list_document_vessel as $res_list_document)
  {
    $count=count($list_document_vessel);
    $document_sl  = $res_list_document['document_sl'];
    $doc_file     =   $this->Function_model->get_doc_file($vessel_id, $document_sl,$survey_id1);
    $document_id  =   @$doc_file->document_id;
    $filename     =   @$doc_file->document_name;

    if($filename==NULL)
    {
      $msg_disp='<font color="#7a29c6"><b> No </b>  </font>&nbsp;<input type="file" name="myFile'.$document_sl.'" id="myFile'.$document_sl.'" onChange="validate_file(this.value,this.id)">';
    }
    else 
    {
      $msg_disp='<font color="#7a29c6"> <a href="'.base_url().'uploads/survey/'.$document_id.'/'.$filename.'" target="_blank" > <b> <img src="'.base_url().'assets/img/pdf_icon.png" width="30" height="30"> </b> </a> </font>&nbsp<input type="file" name="myFile'.$document_sl.'" id="myFile'.$document_sl.'" onChange="validate_file(this.value,this.id)">';
    }*/
  ?>
<!-- <div class="row no-gutters oddtab border-bottom" id="survey1">
<div class="col-6 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1">
<?php  echo $res_list_document['document_name']; ?>

</div> 
<div class="col-6 d-flex justify-content-left mt-1 mb-1 text-primary formfont">
<?php
//  echo $msg_disp; 
  ?>
...
?>
<input type="hidden" value=""<?php //echo $count; ?> name="cntcount" id="cntcount">

<div class="row no-gutters eventab border-bottom" id="survey1">
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1">
Last expired certificate of survey
</div> <!-- end of col-3 1st-->
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-primary formfont">
<input type="file" name="form2_myFile11" id="form2_myFile10" onChange="validate_file(this.value,this.id)">
</div> <!-- end of col-3 2nd-->
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1 border-primary ">
Copy of certificate of registration
</div> <!-- end of col-3 3rd-->
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-primary formfont pl-1">
<input type="file" name="form2_myFile12" id="form2_myFile11" onChange="validate_file(this.value,this.id)">
...
<!-- <div class="row no-gutters oddtab border-bottom" id="survey1">
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1">
In case of agent, authorization from the owners to undertake the survey of the vessel
</div> 
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-primary formfont">
<input type="file" name="form2_myFile12" id="form2_myFile12" onChange="validate_file(this.value,this.id)">
</div> 
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-dark formfont pl-1 border-primary ">
In case of minor, legal documents in respect of legal /natural guardian
</div> 
<div class="col-3 d-flex justify-content-left mt-1 mb-1 text-primary formfont pl-1">
<input type="file" name="form2_myFile13" id="form2_myFile13" onChange="validate_file(this.value,this.id)">
</div> 
</div>  -->
...
<div class="tab-pane fade " id="tab2" role="tabpanel" aria-labelledby="form2annual">
<!-- <form name="form2" id="form2" method="post">   -->

  <?php
$attributes = array("class" => "form-horizontal", "id" => "form2", "name" => "form2", "enctype"=> "multipart/form-data");
echo form_open("Kiv_Ctrl/Survey/form2_annual_survey", $attributes);
?>
...
<div class="row no-gutter  mx-0 postodd">
<div class="col-12 py-2 text-primary px-5">
<?php echo $listname; ?> &nbsp; <a href="<?php echo base_url(); ?>uploads/annualsurvey/<?php echo $Id; ?>/<?php echo $pdfname; ?>" target="_blank"><i class="fas fa-file-pdf h4"></i> </a>
...
6. <?php if($minor_details) {
 ...
 ?> 
<a href="<?php echo base_url(); ?>uploads/guardianship_document/<?php echo $pdfname; ?>" download>In case of minor, legal documents in respect of legal /natural guardian.<img src="<?php echo base_url(); ?>plugins/img/pdf_icon.png" width="30" height="30"> </a>
<?php } else 
{
  ?>
  In case of minor, legal documents in respect of legal /natural guardian.
...
<div class="tab-pane fade " id="tab3" role="tabpanel" aria-labelledby="paymenttab">
<!-- <form name="form3" id="form3" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/Survey/annual_survey_payment' ?>">   -->
  <?php
$attributes = array("class" => "form-horizontal", "id" => "form3", "name" => "form3", "enctype"=> "multipart/form-data");
echo form_open("Kiv_Ctrl/Survey/annual_survey_payment", $attributes);
?>
...
<div class="col-3 border-top border-bottom"><p class="mt-3 mb-3">
  <select class="form-control select2 " name="portofregistry_sl" id="portofregistry_sl"  title="" data-validation="required" readonly>
  <option value="">Select</option>
  <?php   foreach ($portofregistry as $res_portofregistry)
  {
  ?>
  <option value="<?php echo $res_portofregistry['int_portoffice_id']; ?>"<?php if($vessel_registry_port_id==$res_portofregistry['int_portoffice_id']) { echo "selected"; } ?>><?php echo $res_portofregistry['vchr_portoffice_name'];?>  </option>
  <?php    }    ?>
  </select></p>
</div>
...
<div class="row no-gutters oddtab">
<div class="col-3 border-top border-bottom"> <p class="mt-3 mb-3">Amount to be paid</p></div>
<div class="col-3 border-top border-bottom"><p class="mt-3 mb-3"> <input type="text" class="form-control" name="dd_amount" value="<?php echo $amount; ?>" id="dd_amount" maxlength="8" autocomplete="off"  required readonly>
 </p>
</div>
...
<div class="row no-gutters eventab">    
<div class="col-12 d-flex justify-content-end">
<button type="submit" class="btn btn-primary btn-flat  btn-point btn-md" name="tab3next" id="tab3next" ><i class="far fa-save"></i>&nbsp;Save &amp; proceed</button>
</div> <!-- End of button col -->
</div> 
