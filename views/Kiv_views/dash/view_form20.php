...
else
{
  ...
  $registration_intimation            = $this->Vessel_change_model->get_vesselduplicert_details_ra($vessel_id);
...
if(!empty($dupcert_det)){
  foreach($dupcert_det as $res_dup)
  {
    $duplicate_cert_verify_id         = $res_dup['duplicate_cert_verify_id'];
    $duplicate_cert_pc_verified_date  = date("d-m-Y", strtotime($res_dup['duplicate_cert_pc_verified_date']));
    $duplicate_cert_payment_date      = date("d-m-Y", strtotime($res_dup['duplicate_cert_payment_date']));
  } 
}
$verified_usertype                    = $this->Vessel_change_model->get_user_master($duplicate_cert_verify_id);
...
$process=41;
$status=1;
$dupcert_intimation                   = $this->Vessel_change_model->get_dupcert_intimation_det($vessel_id,$survey,$process,$status);
$data['dupcert_intimation']           = $dupcert_intimation; //print_r($ownership_intimation);
if(!empty($dupcert_intimation)){
  foreach($dupcert_intimation as $res_ownintim){
  ...
  <div class="row ui-innerpage" >
    <div class="col-12">
      <div class="container letterform mb-4">
        <form name="form1" id="form1" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/duplicert_intimation_send/'.$vessel_id.'/'.$processflow_sl.'/'.$status_details_sl ?>" enctype="multipart/form-data">
<!-- <form name="form1" action="<?php //echo site_url('VesselChange/namechange_intimation_send/')?>" method="POST" enctype="multipart/form-data" id="form1"> -->
          ...
          <div class="row no-gutters">
            ...
            <div class="col-12 d-flex justify-content-center text-primary"> <u> APPLICATION FOR DUPLICATE CERTIFICATE OF THE VESSEL </u></div>
          </div>
          ...
          <div class="row oddrow py-2">
            ...
            <div class="col-3 px-2 text-secondary"> <?php echo $duplicate_cert_payment_date ; ?></div>
          </div>
          <div class="row evenrow py-2">
            ...
            <div class="col-3 px-2 text-secondary"> <?php echo $duplicate_cert_pc_verified_date ; ?></div>
          </div>
          <div class="row py-2 px-2">
            <div class="col-12 d-flex justify-content-center">
              ...
              <a class="btn btn-flat btn-sm btn-info" href="<?php echo site_url(); ?>/Kiv_Ctrl/VesselChange/form20_certificate/<?php echo $vessel_id1; ?>" target="_blank" width="30" height="30">Form --<i class="fas fa-file-pdf h4"></i> </a> 
            </div> 
          </div>
...
{ 
    ...
    $.ajax({ 
      url : "<?php echo site_url('Kiv_Ctrl/VesselChange/dupcertificate_insertion')?>",
      type: "POST",
      ...
      {//alert(data);exit;
        if(data==1)
        {
          ...
          location.replace('<?php echo site_url('Kiv_Ctrl/VesselChange/generate_dupcertificate')?>/'+vessel_id1+'/'+processflow_sl1+'/'+status_details_sl1);
        }
        if(data==0)
        {
          alert("Failed");
