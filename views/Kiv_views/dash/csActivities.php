            ...
            <?php
                ...
                foreach($data_task as $res1)
                {
                 ...
                 @$status                = $res1['status'];
                  ...
                  $inspection_date1=date("d-m-Y", strtotime($res1['assign_date']));
                  if($inspection_date1=='01-01-1970')
                  {
                    $inspection_date="";
                  }
                  else
                ...
                $defect_count           =   $this->Survey_model->get_defect_count($vessel_sl2,$survey_id);
               ...
               if(!empty($defect_count))
                  ...
                  if($count!=0 && $count>2)
                  {
                   /* $defect_count           =   $this->Survey_model->get_defect_count($vessel_sl,$survey_id);
                    $data['defect_count']   =   $defect_count;*/
                  }
...
$processflow_sl=str_replace(array('+', '/', '='), array('-', '_', '~'), $processflow_sl);
                ...
                <a href="<?php echo $site_url.'/Kiv_Ctrl/Survey/csTask/'.$vessel_sl.'/'.$processflow_sl.'/'.$survey_id2  ?>" class="btn btn-danger btn-xs">Verify</a>
                <?php
              }
           if($status==1 && $current_status_id==7 && $process_id!=15 && $survey_id==1)
              ...
              else if(($status==1 && $current_status_id==7  && $process_id==15) || ($status==1 && $current_status_id==2  && $process_id==15) )
             {
                ?>
                <a href="<?php echo $site_url.'/Kiv_Ctrl/Survey/annualsurvey_form2/'.$vessel_sl.'/'.$processflow_sl.'/'.$survey_id2 ?>" class="btn btn-danger btn-xs">Verify form2</a>
                <?php
              }
