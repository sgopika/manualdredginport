...
echo form_open("Kiv_Ctrl/Survey/drydocksurvey_done", $attributes);
?>
...
<div class="row no-gutters p-1 pb-2 justify-content-center">
  <div class="col-10 text-center">
      <div class="alert bg-darkslateblue" role="alert">
        Completed Drydock Survey Details
      </div> <!-- end of alert -->
  </div> <!-- end of col10 -->
...
<?php if(!empty($drydock_survey_done)) { ?>
<?php if($day=='30') { ?>
...
?>
 ...
 <table id="example" class="table table-bordered table-striped table-hover dataTable no-footer" role="grid" aria-describedby="example1_info">
        <thead>
           <tr><th colspan="8"><font size="4">Drydock survey completed</font></th></tr>
            <tr>
                  <th>#</th>
        ...
        <tbody>
            <?php
                ...
                foreach($drydock_survey_done as $res1)
                {
                  $survey_date= date('d-m-Y',strtotime($res1['actual_date']));
                  $timeline_sl2=$res1['timeline_sl'];
...
$timeline_sl1 = $this->encrypt->encode($timeline_sl2); 
$timeline_sl=str_replace(array('+', '/', '='), array('-', '_', '~'), $timeline_sl1);     
         ...
         ?>
                ...
                <td>
                <a  href="<?php echo site_url(); ?>/Kiv_Ctrl/Survey/drydocksurvey_certificate/<?php echo $vessel_sl.'/'.$timeline_sl; ?>" target="_blank" width="30" height="30"><i class="fas fa-file-pdf h4"></i> </a>
