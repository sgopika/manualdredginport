...
else
{
  ...
  $registration_intimation            = $this->Vessel_change_model->get_vesselnamechange_details_vw($vessel_id);
  ...
  foreach ($registration_intimation as $key ) 
  {
    ...
    $reference_number                 = $key['reference_number'];
    $official_number                  = $key['official_number'];
    $owner_name                       = $key['user_master_fullname'];
    @$owner_address                   = $key['user_address'];
    $vessel_created_timestamp         = $key['vessel_created_timestamp'];
    @$registration_intimation_sl      = $key['registration_intimation_sl'];
  }
    $id                               = $registration_intimation[0]['vessel_created_user_id'];
    ...
    if(!empty($customer_details)) 
  {
    ...
    @$vessel_registry_port_id         = $key['registration_intimation_place_id'];
    @$registration_intimation_sl      = $key['registration_intimation_sl'];
    if(isset($vessel_registry_port_id)){
    $registry_port_id                 = $this->Survey_model->get_registry_port_id($vessel_registry_port_id);
    $data['registry_port_id']         = $registry_port_id;}
}
...
if(!empty($vessel_change_det)){
  ...
  $change_req_date                    = date('d-m-Y',strtotime($vessel_change_det[0]['change_req_date']));
  $change_payment_date                = date('d-m-Y',strtotime($vessel_change_det[0]['change_payment_date']));
  $change_verify_id                   = $vessel_change_det[0]['change_verify_id'];
  
  $change_pc_verified_date            = date('d-m-Y',strtotime($vessel_change_det[0]['change_pc_verified_date']));
  ...
  <div class="row ui-innerpage" >
    <div class="col-12">
      <div class="container letterform mb-4">
        <form name="form1" id="form1" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/namechange_intimation_send/'.$vessel_id.'/'.$processflow_sl.'/'.$status_details_sl ?>" enctype="multipart/form-data">
<!-- <form name="form1" action="<?php //echo site_url('VesselChange/namechange_intimation_send/')?>" method="POST" enctype="multipart/form-data" id="form1"> -->
        ...
        <input type="hidden" name="change_name" id="change_name" value="<?php echo $change_name; ?>">
        ...
        <div class="row no-gutters">
          <div class="col-12 d-flex justify-content-center text-primary">&nbsp; Form 11</div>
          <div class="col-12 d-flex justify-content-center text-primary"> <u> APPLICATION FOR CHANGE OF NAME OF THE VESSEL </u></div>
        </div>
        ...
        <div class="row evenrow py-2">
          <div class="col-3 pl-2 text-primary"> Vessel New Name</div>
          <div class="col-3 px-2 text-secondary"> <?php echo $change_name; ?></div>
          <div class="col-3 pl-2 text-primary"> New Name Requested Date </div>
          <div class="col-3 px-2 text-secondary"> <?php echo $change_req_date ; ?></div>
        </div>
        <div class="row oddrow py-2">
          ...
          <div class="col-3 px-2 text-secondary"> <?php echo $change_payment_date ; ?></div>
        </div>
        <div class="row evenrow py-2">
          ...
          <div class="col-3 px-2 text-secondary"> <?php echo $change_pc_verified_date ; ?></div>
        </div>
        <div class="row py-2 px-2">
          <div class="col-12 d-flex justify-content-center">
            ...
            <?php }?>
            <?php if($category==2){?>
            <a class="btn btn-flat btn-sm btn-info" href="<?php echo site_url(); ?>/Survey/form10_certificate/<?php echo $vessel_id; ?>" target="_blank" width="30" height="30">Certificate of Survey<i class="fas fa-file-pdf h4"></i> </a> &nbsp;
            <?php }?>
            ...
            <a class="btn btn-flat btn-sm btn-info" href="<?php echo site_url(); ?>/Kiv_Ctrl/VesselChange/form11_certificate/<?php echo $vessel_id1; ?>" target="_blank" width="30" height="30">Form 11<i class="fas fa-file-pdf h4"></i></a> 
          </div> 
        </div>
        ...
        <div id="new_intimation" style="display: none;">
          <div class="row no-gutters oddrow border-bottom">
            ...
            <div class="col-3 d-flex justify-content-left mt-1 mb-1 text-primary formfont">
              <input type="date" class="form-control dob" id="change_inspection_date" name="change_inspection_date" placeholder="Inspection Date" data-validation="required" value="<?php if(isset($date)){ echo date('m/d/Y',strtotime($date));}?>">
            </div> 
...
$(document).ready(function()
{
    ...
    $("#generatecertificate").click(function()
    {
      ...
      $.ajax({ 
        url : "<?php echo site_url('Kiv_Ctrl/VesselChange/form14_form15_insertion')?>",
        type: "POST",
        ...
        {//alert(data);exit;
          if(data==1)
          {
            ...
            location.replace('<?php echo site_url('Kiv_Ctrl/VesselChange/generate_certificate')?>/'+vessel_id1+'/'+processflow_sl1+'/'+status_details_sl1);
          }
          if(data==0)
          {
            alert("Failed");
