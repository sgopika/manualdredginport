...
else
{
  ...
  $registration_intimation          = $this->Vessel_change_model->get_transfervessel_details_ra($vessel_id);
...
if(!empty($transfervsl_details)){
  foreach($transfervsl_details as $res_tsfrvsl)
  {
    ...
    if($transfer_based_changetype==1){
      ...
      $transfer_pc_verified_date    = date("d-m-Y", strtotime($res_tsfrvsl['transfer_pc_verified_date']));
      $transfer_payment_date        = date("d-m-Y", strtotime($res_tsfrvsl['transfer_payment_date']));
      $transfer_verify_id           = $res_tsfrvsl['transfer_verify_id'];
      $transfer_buyer_name          = '';
...
$process                            = 40;
$status                             = 1;
$ownership_intimation               = $this->Vessel_change_model->get_transfervessel_intimation_det($vessel_id,$survey,$process,$status);
  ...
  <div class="row ui-innerpage" >
    <div class="col-12">
      <div class="container letterform mb-4">
        <form name="form1" id="form1" method="post" action="<?php echo $site_url.'/Kiv_Ctrl/VesselChange/transfervessel_intimation_send/'.$vessel_id.'/'.$processflow_sl.'/'.$status_details_sl ?>" enctype="multipart/form-data">
<!-- <form name="form1" action="<?php //echo site_url('VesselChange/namechange_intimation_send/')?>" method="POST" enctype="multipart/form-data" id="form1"> -->
          ...
          <input type="hidden" name="transfer_based_changetype" id="transfer_based_changetype" value="<?php echo $transfer_based_changetype; ?>">
          ...
          <input type="hidden" name="transfer_portofregistry_from" id="transfer_portofregistry_from" value="<?php echo $transfer_portofregistry_from; ?>">
          <input type="hidden" name="transfer_portofregistry_to" id="transfer_portofregistry_to" value="<?php echo $transfer_portofregistry_to; ?>">
          <input type="hidden" name="state_nm" id="state_nm" value="<?php echo $state_nm; ?>">
          ...
          <div class="row no-gutters">
            ...
            <div class="col-12 d-flex justify-content-center text-primary"> <u> APPLICATION FOR TRANSFER OF THE VESSEL </u></div>
          </div>
          ...
          <div class="row py-2 px-2">
            <div class="col-12 d-flex justify-content-center">
              ...
              <a class="btn btn-flat btn-sm btn-info" href="<?php echo site_url(); ?>/Kiv_Ctrl/VesselChange/form19_certificate/<?php echo $vessel_id1; ?>" target="_blank" width="30" height="30">Form --<i class="fas fa-file-pdf h4"></i> </a> 
            </div> 
          </div>
          ...
          <div id="form14" style="display: none;">
            <div class="row evenrow py-2">
              ...
              <div class="col-3 px-2 text-secondary">  <select class="form-control select2"  name="approve_status" id="approve_status">
...
$(document).ready(function()
{
  ...
  { 
    ...
    var buyer_id            = $("#transfer_buyer_id").val();  //alert(buyer_id);
    var transfer_change     = $("#transfer_based_changetype").val();  //alert(transfer_change);
    var vessel_id           = btoa(vessel_id1);
    var processflow_sl      = btoa(processflow_sl1);
    var status_details_sl   = btoa(status_details_sl1);
    
    if((transfer_change==3) && (buyer_id==0)){ 
      
        alert("Buyer is not a Registered User!!!");
      
    } else {

      $.ajax({ 
        url : "<?php echo site_url('Kiv_Ctrl/VesselChange/form_transfervessel_insertion')?>",
        type: "POST",
        ...
        {//alert(data);exit;
         if(data==1)
         {
          ...
          location.replace('<?php echo site_url('Kiv_Ctrl/VesselChange/generate_certificate_tran')?>/'+vessel_id1+'/'+processflow_sl1+'/'+status_details_sl1);} else { location.replace('<?php echo site_url('Kiv_Ctrl/Bookofregistration/raHome')?>'); }
         }
         if(data==0)
         {
             alert("Failed");
